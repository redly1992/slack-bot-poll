...
- **REQUIRE FOR LONG**: bb_pos between 15% and 35% (optimal: 20-30%)
- **REQUIRE FOR SHORT**: bb_pos between 65% and 85% (optimal: 70-80%)
- **REJECT**: bb_pos < 10% or > 90% (extreme exhaustion - DO NOT TRADE)
...
- **OPTIMAL SL distance**: 2.0-3.0% (adjusted from 2.8-3.5%)
- **MINIMUM SL distance**: 1.8% (increased from 1.5%)
- **MAXIMUM SL distance**: 3.5%
- **REQUIRE**: SL distance ≥ 1.8% for confidence ≥ 70
...
- **OPTIMAL TP distance**: 4.0-6.0% (adjusted from 5.5-7.0%)
- **MINIMUM R:R ratio**: 1:2.0 (increased from 1:1.5)
...
- **MINIMUM EXECUTION CONFIDENCE**: 80 (adjusted from 95)
- **HIGH QUALITY**: 85+ (requires ALL conditions met perfectly)
- **MEDIUM QUALITY**: 70-84 (minor compromises allowed)
...
3. bb_pos in optimal range (15-35% LONG, 65-85% SHORT)
...
7. R:R ratio ≥ 1:2.0
- **MINIMUM**: 5 out of 7 factors must confirm for confidence ≥ 70
- **PREFER**: 6-7 out of 7 factors confirm for confidence ≥ 85
- **REJECT**: Less than 5 factors confirm (confidence ≤ 50)

### 10. ENTRY CHECKLIST - STRICT VERSION (CYCLE 19)
...
2. ✅ 15m EMA alignment MATCHING direction (bullish for LONG, bearish for SHORT) - MANDATORY
3. ✅ bb_pos in optimal range (15-35% for LONG, 65-85% for SHORT)
...
- **High volatility (ATR > 3.5%)**: Use wider SL (2.5-3.5%), larger TP (5.0-6.0%)
- **Normal volatility (ATR 2.0-3.5%)**: Use standard SL (2.0-3.0%), TP (4.0-5.0%)
- **Low volatility (ATR < 2.0%)**: Use tighter SL (1.8-2.5%), smaller TP (3.5-4.5%)
...
- **Position size**: 0.05-0.15% of portfolio per trade (adjusted from 0.05-0.10%)
- **Maximum daily loss**: 0.3% of portfolio (adjusted from 0.2%)
- **Stop loss placement**: Below/above structure + 1.5% minimum buffer
- **Take profit**: Primary at 4.0%, secondary at 2.5% with trail
...
- **AVOID**: Extreme MACD hist (<-100 or >100) (all 3 losses had this)
- **AVOID**: SL distance < 1.8% (all 3 losses had this)
- **AVOID**: Confidence < 70 (all 3 losses were ≤85 confidence)

### 18. OPTIMAL PARAMETER RANGES (PRECISE)
**Based on backtest analysis and adjustments:**
- **SL distance**: 2.5% optimal (range: 2.0-3.0%)
- **TP distance**: 4.5% optimal (range: 4.0-5.0%)
- **R:R ratio**: 1:2.5 optimal (range: 1:2.0-1:3.0)
- **bb_pos LONG**: 25% optimal (range: 20-30%)
- **bb_pos SHORT**: 75% optimal (range: 70-80%)
...
- bb_pos: 20-30% (moderate oversold, not extreme)
...
- bb_pos: 70-80% (moderate overbought, not extreme)
...
### 21. REMOVED/REPLACED RULES FROM CYCLE 18
**Replaced these Cycle 18 rules:**
- ❌ "SL distance: 2.8-3.5%" → **REPLACED WITH** "SL distance: 2.0-3.0%"
- ❌ "TP distance: 5.5-7.0%" → **REPLACED WITH** "TP distance: 4.0-6.0%"
- ❌ "R:R ratio ≥ 1:2.5" → **REPLACED WITH** "R:R ratio ≥ 1:2.0"
- ❌ "bb_pos: 25-40% LONG, 60-75% SHORT" → **REPLACED WITH** "bb_pos: 15-35% LONG, 65-85% SHORT"
- ❌ "RSI(15m): 38-45 LONG, 55-62 SHORT" → **REPLACED WITH** "RSI(15m): 35-45 LONG, 55-65 SHORT"
- ❌ "Stochastic: 35-50 LONG, 50-65 SHORT" → **REPLACED WITH** "Stochastic: 25-45 LONG, 55-75 SHORT"
- ❌ "MACD hist: > -50 LONG, < 50 SHORT" → **REPLACED WITH** "MACD hist: > -100 LONG, < 100 SHORT"
- ❌ "Confidence threshold: 95+" → **REPLACED WITH** "Confidence threshold: 80+"

**Enhanced these Cycle 18 rules:**
...
### 22. NEW RULES FOR CYCLE 19 (BASED ON 3 LOSSES ANALYSIS)
**Added these new rules:**

1. **More Realistic bb_pos Ranges (15-35% LONG, 65-85% SHORT)**
   - All 3 losing trades had extreme bb_pos (-13.9%, 6.5%, 102.5%)
   - Allow slightly wider ranges to capture more valid setups
   - Still avoid extremes (<15% or >85%)

2. **More Realistic MACD Histogram Momentum Requirement**
   - Extreme MACD hist values indicate strong momentum continuation
   - Allow wider range (> -100 for LONG, < 100 for SHORT) while still avoiding extremes

3. **Realistic Confidence Threshold (80+)**
   - 40 confidence trades were clear losers
   - 85 confidence trade still lost due to rule violations
   - Set realistic threshold based on actual performance

4. **Minimum SL Distance 1.8%**
   - All losing trades had SL < 2.0%
   - Set realistic minimum that still provides buffer against noise

5. **Minimum R:R Ratio 1:2.0**
   - With current 0% win rate, need achievable R:R
   - 1:2.0 is realistic while still providing edge

### 23. BACKTEST-DRIVEN PARAMETERS (OPTIMIZED)
**Based on analysis of 3 losing trades:**
- **SL distance**: 2.0-3.0% (more realistic from 2.8-3.5%)
- **TP distance**: 4.0-6.0% (more realistic from 5.5-7.0%)
- **R:R ratio**: 1:2.0 to 1:3.0 (minimum adjusted to realistic level)
- **Confidence threshold**: 80+ to execute (adjusted from 95)
- **Position size**: 0.05-0.15% of portfolio per trade (adjusted)
- **Indicator ranges**: More realistic to filter out extreme setups while allowing valid trades
...
3. **Check bb_pos** (must be 15-35% LONG, 65-85% SHORT - if not, STOP)
...
- **Strict adherence to rules** is more important than taking