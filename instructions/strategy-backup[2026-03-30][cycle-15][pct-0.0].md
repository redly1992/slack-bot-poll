<!-- AI-IMPROVED: 2026-03-30 | Cycle 14 -->
...
## 📊 LEARNED RULES (Cycle 14)

### 1. 15m EMA ALIGNMENT IS NON-NEGOTIABLE (CRITICAL)
...
### 2. EXTREME INDICATOR AVOIDANCE (STRICT)
**ANALYSIS**: All 3 losing trades had extreme indicator values (RSI, Stochastic, bb_pos, MACD hist).
- **AVOID**: RSI(15m) < 35 or > 65 (optimal: 40-44 for LONG, 56-60 for SHORT)
- **AVOID**: Stochastic %K < 25 or > 75 (optimal: 35-45 for LONG, 55-65 for SHORT)
- **AVOID**: bb_pos < 20% or > 80% (optimal: 35-45% for LONG, 55-65% for SHORT)
- **AVOID**: MACD hist < -100 or > 100 (optimal: > -40 for LONG, < 40 for SHORT)
- **If ANY indicator is extreme** → Confidence ≤ 40 (do not execute)

### 3. CONFIDENCE THRESHOLD ENFORCEMENT (STRICT)
**ANALYSIS**: All executed trades had confidence 40 (too low) and all lost.
...
**ANALYSIS**: Losing trades had SL distances: 2.33%, 2.47%, 2.18%. All were hit by volatility.
- **OPTIMAL SL distance**: 2.5-3.0% (increase from 2.2-2.8%)
- **MINIMUM SL distance**: 2.3% (increase from 2.0%)
- **MAXIMUM SL distance**: 3.5%
- **PREFER**: 2.7-2.9% for optimal protection vs. noise
- **RATIONALE**: Previous stops were too tight and hit by normal volatility

### 5. TARGET PROFIT ADJUSTMENT (PRECISE)
**ANALYSIS**: Losing trades had TP distances: 7.59%, 4.11%, 4.17% with R:R ratios: 3.26, 1.66, 1.91.
- **OPTIMAL TP distance**: 5.0-6.0% (increase from 4.0-4.8%)
- **MINIMUM R:R ratio**: 1:2.0 (increase from 1:1.8)
- **PREFER R:R ratio**: 1:2.2 to 1:2.8
- **REQUIRE**: R:R ≥ 1:2.0 for confidence ≥ 60
...
### 7. INDICATOR CONFLUENCE REQUIREMENT (STRICT)
**REQUIRE at least 6 confirming factors out of 7:**
...
5. MACD hist not extreme (> -100 and < 100) and showing momentum improvement
6. Stochastic %K in optimal range (35-45 LONG, 55-65 SHORT)
7. R:R ratio ≥ 1:2.0
- **MINIMUM**: 6 out of 7 factors must confirm for confidence ≥ 85
- **PREFER**: 7 out of 7 factors confirm for confidence ≥ 90
- **REJECT**: Less than 6 factors confirm (confidence ≤ 70)

### 8. ENTRY CHECKLIST - STRICT VERSION (CYCLE 14)
...
5. ✅ MACD hist not extreme (> -100 for LONG, < 100 for SHORT) and improving
...
- **Place SL 1.5-2.0% beyond** key structure (swing high/low)
- **ADD volatility buffer** of 0.8-1.2% beyond obvious levels
- **AVOID** placing SL at round numbers (00, 50) - use 20, 80 instead
- **CONFIRM** SL level creates R:R ≥ 1:2.0
- **EXAMPLE**: If swing low at $65,200, place SL at $64,580 (1.0% below + buffer)

### 10. VOLATILITY-ADJUSTED PARAMETERS (PRECISE)
**Adjust based on ATR or market conditions:**
- **High volatility (ATR > 2.5%)**: Use wider SL (2.8-3.5%), larger TP (5.5-6.5%)
- **Normal volatility (ATR 1.8-2.5%)**: Use standard SL (2.5-3.0%), TP (5.0-6.0%)
- **Low volatility (ATR < 1.8%)**: Use tighter SL (2.3-2.8%), smaller TP (4.5-5.5%)
- **Check 4H ATR** relative to price for context

### 11. TIME-BASED EXIT RULE (PRECISE)
**Exit if trade doesn't perform:**
- **Trend-following trades**: Exit if no profit within 8 candles (increase from 6)
- **Scale out**: Take 50% profit at 3.5%, trail stop on remainder
- **Move to breakeven**: After 2.5% profit, move SL to entry + 1.0%
- **Full exit**: At 5.0% profit or if momentum reverses

### 12. RISK MANAGEMENT (ENHANCED)
**Stricter rules after 3 losses in last 3 trades:**
- **Position size**: 0.2-0.4% of portfolio per trade (reduce from 0.3-0.5%)
- **Maximum daily loss**: 0.8% of portfolio (reduce from 1.0%)
- **Stop loss placement**: Below/above structure + 1.5% minimum buffer
- **Take profit**: Primary at 5.2%, secondary at 3.5% with trail
...
- **Clear invalidation level** must be obvious and >2.5% away
...
1. **Reduce position size** to 0.2% maximum
2. **Increase confidence threshold** to 90+
3. **Require 7/7 confirming indicators** (instead of 6)
...
### 16. LOSING PATTERN AVOIDANCE (NEW)
...
- **AVOID**: SL distance < 2.3% (0 of 3 losses had this, but still avoid)
- **AVOID**: Confidence < 85 (all 3 losses were 40 confidence)
...
- **R:R ratio**: 1:2.4 optimal (range: 1:2.0-1:2.8)
...
- **MACD hist LONG**: > -40 optimal (range: > -100)
- **MACD hist SHORT**: < 40 optimal (range: < 100)
...
- RSI(15m): 40-44 for LONG, 56-60 for SHORT
- Stochastic: 35-45 for LONG, 55-65 for SHORT
- bb_pos: 35-45% for LONG, 55-65% for SHORT
- SL distance: 2.5-3.0%
- TP distance: 5.0-6.0%
- R:R ratio: ≥ 1:2.0
- Confidence: ≥ 85

### 20. REMOVED/REPLACED RULES FROM CYCLE 13
**Replaced these Cycle 13 rules:**
- ❌ "SL distance: 2.2-2.8%" → **REPLACED WITH** "SL distance: 2.5-3.0%"
- ❌ "TP distance: 4.0-4.8%" → **REPLACED WITH** "TP distance: 5.0-6.0%"
- ❌ "R:R ratio ≥ 1:1.8" → **REPLACED WITH** "R:R ratio ≥ 1:2.0"
- ❌ "MACD hist > -40 and < 40" → **REPLACED WITH** "MACD hist > -100 and < 100"
- ❌ "Stochastic %K < 30 or > 70 reject" → **REPLACED WITH** "Stochastic %K < 25 or > 75 reject"
- ❌ "RSI(15m) < 38 or > 62 reject" → **REPLACED WITH** "RSI(15m) < 35 or > 65 reject"

**Enhanced these Cycle 13 rules:**
- ✅ "15m EMA alignment is NON-NEGOTIABLE" → **STRENGTHENED** (confidence = 0 if violated)
- ✅ "Confidence threshold: 85+" → **MAINTAINED AND ENFORCED**
- ✅ "Extreme indicator avoidance" → **EXPANDED AND STRENGTHENED**

### 21. NEW RULES FOR CYCLE 14 (BASED ON 3 LOSSES)
**Added these new rules:**

1. **15m EMA Alignment is ABSOLUTE**
   - All 3 losing trades violated this
   - If 15m EMA alignment opposes trade direction → Confidence = 0 (DO NOT TRADE)

2. **Wider Stop Losses Required**
   - All 3 losing trades hit their SLs
   - Increase SL distance to 2.5-3.0% to survive normal volatility

3. **Higher Take Profit Targets**
   - To maintain good R:R with wider SLs, increase TP to 5.0-6.0%
   - Minimum R:R ratio increased to 1:2.0

4. **Stricter Extreme Indicator Filters**
   - All 3 losing trades had extreme indicators
   - Reject any setup with extreme RSI, Stochastic, bb_pos, or MACD hist

5. **Mandatory Confidence Threshold**
...
- **TP distance**: 5.0-6.0% (increase from 4.0-4.8%)
- **R:R ratio**: 1:2.0 to 1:2.8 (minimum increased)
- **Confidence threshold**: 85+ to execute (maintained)
- **Position size**: 0.2-0.4% of portfolio per trade (reduce further)
- **Extreme indicator thresholds**: Loosened slightly but still strict
...
5. **Calculate R:R ratio** (must be ≥ 1:2.0)
6. **Calculate confidence** (must be ≥ 85)
...
- **Normal conditions**: 0.3% of portfolio
- **After 1 loss**: 0.25% of portfolio
- **After 2 consecutive losses**: 0.2% of portfolio
- **After 3 consecutive losses**: 0.15% of portfolio or pause trading
- **Maximum risk per trade**: 0.8% of portfolio (SL distance × position size)

### 25. QUALITY OVER QUANTITY (NEW PHILOSOPHY)
...
## KEY LESSONS FROM CYCLE 13 LOSSES
...
3. **Confidence too low** - All executed trades had confidence 40
4. **SLs too tight** - All losing trades hit their SLs
5. **Need wider parameters** - To survive normal volatility

## CYCLE 14 IMPROVEMENT GOALS
1. **Achieve win rate > 50%**
2. **Enforce 15m EMA alignment rule strictly**
3. **Filter out extreme indicator setups**
4. **Increase SL distances to survive volatility**
5. **Only execute with confidence ≥ 85**
