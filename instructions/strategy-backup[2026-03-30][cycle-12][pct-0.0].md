...
### 1. 15m EMA ALIGNMENT MUST SUPPORT TRADE DIRECTION (CRITICAL)
**ANALYSIS**: All 3 losing trades had 15m EMA alignment OPPOSING the trade direction (bearish for LONG, bullish for SHORT). This is the #1 failure pattern.
- **REQUIRE**: 15m EMA alignment must be NEUTRAL or FAVORING trade direction
- **STRICT REJECT**: If 15m EMA alignment strongly opposes trade (bullish for SHORT, bearish for LONG) → Confidence ≤ 40
- **PREFER**: 15m EMA alignment matching trade direction for confidence ≥ 75
- **CONFIRMATION**: Check 15m ema_fast vs ema_slow before entry

### 2. BOLLINGER BAND POSITION MODERATE ZONES ONLY (STRICT)
**ANALYSIS**: Losing trades had extreme bb_pos values (6.5%, -13.9%, 102.5%). Extreme positions indicate momentum exhaustion but often continue beyond extremes.
- **For LONG entries**: bb_pos between 30% and 50% (optimal: 35-45%)
- **For SHORT entries**: bb_pos between 50% and 70% (optimal: 55-65%)
- **STRICT REJECT**: bb_pos < 20% or > 80% → Confidence ≤ 40
- **AVOID**: Entering at extreme Bollinger Band positions (touching bands)

### 3. RSI MODERATE OVERSOLD/OVERBOUGHT ZONES (PRECISE)
**ANALYSIS**: Losing LONG trades had RSI(15m) at 24.2 and 30.0 (too oversold). Losing SHORT had RSI(15m) at 76.6 (too overbought).
- **For LONG entries**: RSI(15m) between 38 and 44 (moderate oversold)
- **For SHORT entries**: RSI(15m) between 56 and 62 (moderate overbought)
- **STRICT REJECT**: RSI(15m) < 35 or > 65 → Confidence ≤ 40
- **PREFER**: RSI(15m) 40-42 for LONG, 58-60 for SHORT

### 4. STOP LOSS DISTANCE OPTIMIZATION (INCREASED)
**ANALYSIS**: Losing trades had SL distances: 1.88%, 2.04%, 2.18%. All were hit by volatility.
- **OPTIMAL SL distance**: 2.0-2.5% (increase from 1.8-2.2%)
- **MINIMUM SL distance**: 1.8% (increase from 1.6%)
...
**ANALYSIS**: Losing trades had TP distances: 2.97%, 2.96%, 3.02% with R:R ratios: 1.58, 1.45, 1.39.
- **OPTIMAL TP distance**: 3.5-4.0% (increase from 3.0-3.5%)
- **MINIMUM R:R ratio**: 1:1.8 (maintain)
- **PREFER R:R ratio**: 1:2.0 to 1:2.5
- **REQUIRE**: R:R ≥ 1:1.5 for confidence ≥ 60
...
- **For LONG entries**: MACD hist > -30 and improving (less negative or positive)
- **For SHORT entries**: MACD hist < 30 and deteriorating (less positive or negative)
- **REJECT**: MACD hist < -50 (strong bearish momentum for LONG) or > 50 (strong bullish momentum for SHORT)
...
- **For LONG entries**: %K between 30 and 45 (oversold but not extreme)
- **For SHORT entries**: %K between 55 and 70 (overbought but not extreme)
- **REJECT**: %K < 25 (too oversold) or > 75 (too overbought)
- **PREFER**: %K crossing %D in moderate zones (35-40 for LONG, 60-65 for SHORT)

### 8. CONFIDENCE THRESHOLD INCREASE (STRICT)
**ANALYSIS**: Losing trades had confidence 45, 45, 78. Medium confidence (78) still produced losses.
- **MINIMUM EXECUTION CONFIDENCE**: 80 (increase from 75)
- **HIGH QUALITY**: 85+ (requires most conditions met)
- **MEDIUM QUALITY**: 80-84 (acceptable with minor compromises)
- **LOW QUALITY**: 40-79 (avoid - do not execute)
...
2. 15m EMA alignment neutral or favoring direction
3. bb_pos in optimal range (30-50% LONG, 50-70% SHORT)
4. RSI(15m) in optimal range (38-44 LONG, 56-62 SHORT)
5. MACD hist not extreme and showing momentum improvement
6. Stochastic %K in optimal range (30-45 LONG, 55-70 SHORT)
7. SL distance 2.0-2.5%
8. R:R ratio ≥ 1:1.8
- **MINIMUM**: 6 out of 8 factors must confirm for confidence ≥ 80
- **PREFER**: 7+ out of 8 factors confirm for confidence ≥ 85
- **REJECT**: Less than 6 factors confirm (confidence ≤ 70)

### 10. ENTRY CHECKLIST - STRICT VERSION (CYCLE 11)
...
3. ✅ bb_pos in optimal range (30-50% for LONG, 50-70% for SHORT)
4. ✅ RSI(15m) in optimal range (38-44 for LONG, 56-62 for SHORT)
5. ✅ MACD hist not extreme (> -50 and < 50) and showing momentum improvement
6. ✅ Stochastic %K in optimal range (30-45 for LONG, 55-70 for SHORT)
7. ✅ SL distance 2.0-2.5%
...
- **ADD volatility buffer** of 0.5-0.8% beyond obvious levels
- **AVOID** placing SL at round numbers (00, 50) - use 20, 80 instead
- **CONFIRM** SL level creates R:R ≥ 1:1.8
- **EXAMPLE**: If swing low at $65,200, place SL at $64,880 (0.5% below + buffer)

### 12. VOLATILITY-ADJUSTED PARAMETERS (PRECISE)
**Adjust based on ATR or market conditions:**
- **High volatility (ATR > 2.0%)**: Use wider SL (2.2-2.8%), larger TP (3.8-4.5%)
- **Normal volatility (ATR 1.2-2.0%)**: Use standard SL (2.0-2.5%), TP (3.5-4.0%)
- **Low volatility (ATR < 1.2%)**: Use tighter SL (1.8-2.2%), smaller TP (3.2-3.7%)
- **Check 4H ATR** relative to price for context

### 13. TIME-BASED EXIT RULE (PRECISE)
**Exit if trade doesn't perform:**
- **Trend-following trades**: Exit if no profit within 8 candles
- **Scale out**: Take 50% profit at 2.5%, trail stop on remainder
- **Move to breakeven**: After 1.5% profit, move SL to entry + 0.5%
- **Full exit**: At 3.5% profit or if momentum reverses
...
- **EXCEPTION**: None in Cycle 11 - pure trend-following only
...
- **SL distance**: 2.0-2.5% (increase from 1.8-2.2%)
- **TP distance**: 3.5-4.0% (increase from 3.0-3.5%)
- **R:R ratio**: 1:1.8 to 1:2.5 (minimum maintained)
- **Confidence threshold**: 80+ to execute (increase from 75+)
- **Position size**: 0.4-0.6% of portfolio per trade (reduce from 0.5-0.8%)

### 16. RISK MANAGEMENT (ENHANCED)
**Stricter rules after 3 consecutive losses:**
- **Position size**: 0.4-0.6% of portfolio per trade (reduce further)
- **Maximum daily loss**: 1.2% of portfolio (reduce from 1.5%)
- **Stop loss placement**: Below/above structure + 1.0% minimum buffer
- **Take profit**: Primary at 3.8%, secondary at 2.5% with trail
- **Maximum consecutive losses**: 2 trades → pause trading for 72 hours
...
- **Clear invalidation level** must be obvious and >2.0% away
...
3. **Price position relative to Bollinger Bands** (30-70% rule)
4. **RSI level** (optimal ranges 38-44/56-62)
5. **MACD momentum** (not extreme, improving)
6. **Stochastic position** (optimal ranges 30-45/55-70)
- **If #1 and #2 conflict** → Do not trade (confidence ≤ 40)
- **If 3+ indicators conflict** → Do not trade (confidence ≤ 50)

### 19. PERFORMANCE RECOVERY STRATEGY (STRICT)
**After 3 consecutive losses in Cycle 10:**
1. **Reduce position size** to 0.4% maximum
2. **Increase confidence threshold** to 85+
3. **Require 7+ confirming indicators** (instead of 6)
4. **Tighten parameter ranges** (RSI 40-42 for LONG, 58-60 for SHORT)
...
- **RSI(15m) < 35 or > 65** (too extreme)
- **Stochastic %K < 25 or > 75** (too extreme)
- **bb_pos < 20% or > 80%** (too extreme)
- **MACD hist < -50 or > 50** (too extreme)
- **15m EMA alignment opposing trade** (critical failure)
- **SL distance < 1.8%** (too tight)
- **R:R ratio < 1:1.5** (insufficient reward)
...
1. **15m EMA alignment opposing trade direction** (all 3 losses - #1 issue)
2. **Extreme indicator readings** (bb_pos, RSI, Stochastic all at extremes)
3. **Stop loss too tight** (all 3 stopped out at 1.9-2.2%)
4. **Entering at Bollinger Band extremes** (touching bands instead of moderate zones)

## CYCLE 11 IMPROVEMENT FOCUS:
1. **STRICT 15m EMA alignment rule** - must support trade direction
2. **Avoid extreme indicator readings** - use moderate zones only
3. **Wider stops** - 2.0-2.5% minimum
4. **Higher confidence threshold** - 80+ only
5. **Stricter confluence** - 6+ confirming indicators required
6. **Better R:R ratios** - target 1:2.0 to 1:2.5