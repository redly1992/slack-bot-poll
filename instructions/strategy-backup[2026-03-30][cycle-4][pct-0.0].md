...
### 1. MANDATORY 15m EMA ALIGNMENT MATCH (CRITICAL)
...
### 2. AVOID EXTREME INDICATOR READINGS (REQUIRED)
...
### 3. OPTIMAL INDICATOR RANGES FOR HIGH-PROBABILITY ENTRIES
**REQUIRE these moderate ranges for high-probability entries:**
- **LONG entries**: RSI(15m) 32-42, Stochastic %K 25-40, bb_pos -5% to 20%
- **SHORT entries**: RSI(15m) 58-68, Stochastic %K 60-75, bb_pos 80% to 95%
- **PREFER** entries where price is within Bollinger Bands, not outside them

### 4. 4H MACD CONFIRMATION REQUIREMENT
**REQUIRE 4H MACD histogram to support trade direction:**
- **For LONG**: 4H MACD hist > -50 (not bearish)
- **For SHORT**: 4H MACD hist < 50 (not bullish)
- **AVOID** entering LONG when 4H MACD hist < -100 (strong downtrend momentum)
- **AVOID** entering SHORT when 4H MACD hist > 100 (strong uptrend momentum)
- **ANALYSIS**: Losing LONG trades had 4H MACD hist at -408.97 and -58.86

### 5. STOP LOSS DISTANCE OPTIMIZATION
**Use these SL distance guidelines:**
- **OPTIMAL SL distance**: 0.9-1.2% (target 1.0%)
- **MINIMUM SL distance**: 0.85% (tight stops get hit by noise)
- **MAXIMUM SL distance**: 1.3% (wide stops increase risk per trade)
- **REJECT** trades with SL distance < 0.85% or > 1.3%
- **ANALYSIS**: Losing trades had SL distances of 0.59%, 0.75%, and 1.46% - two outside optimal range

### 6. TARGET PROFIT DISTANCE OPTIMIZATION
**Use these TP distance guidelines:**
- **OPTIMAL TP distance**: 1.8-2.2% (target 2.0%)
- **MINIMUM R:R ratio**: 1:1.8 (prefer 1:2.0+)
- **AVOID** excessive R:R > 1:3.0 (indicates unrealistic targets)
- **ANALYSIS**: Losing trades had R:R ratios of 2.43, 2.26, and 5.17 - the 5.17 ratio was unrealistic

### 7. PRICE-EMA RELATIONSHIP REQUIREMENT
...
### 8. 4H RSI CONFIRMATION REQUIREMENT
**REQUIRE 4H RSI to support trade direction:**
- **For LONG**: 4H RSI > 45 (bullish momentum present)
- **For SHORT**: 4H RSI < 55 (bearish momentum present)
- **AVOID** entering LONG when 4H RSI < 40 (weak momentum)
- **AVOID** entering SHORT when 4H RSI > 60 (strong momentum)
- **ANALYSIS**: Losing LONG trades had 4H RSI at 43.1 and 45.4 - borderline weak

### 9. ENTRY CHECKLIST (ALL REQUIRED FOR TRADE)
...
3. ✅ 4H MACD histogram supports direction (> -50 for LONG, < 50 for SHORT)
4. ✅ 4H RSI supports direction (> 45 for LONG, < 55 for SHORT)
5. ✅ Price within Bollinger Bands (bb_pos between -5% and 95%)
6. ✅ SL distance 0.9-1.2%
7. ✅ R:R ratio between 1:1.8 and 1:3.0

**If ANY condition fails → REJECT the trade**

### 10. CONFIDENCE SCORING ADJUSTMENTS
**Apply these confidence modifiers:**
- **+30 points** if 15m EMA alignment matches direction (MANDATORY - without this, confidence = 0)
- **+20 points** if all indicators in optimal ranges
- **+15 points** if 4H MACD histogram strongly supports direction (> 0 for LONG, < 0 for SHORT)
- **+10 points** if 4H RSI strongly supports direction (> 50 for LONG, < 50 for SHORT)
- **-100 points** if 15m EMA alignment contradicts direction (REJECT)
- **-30 points** if any indicator in extreme range
- **-20 points** if 4H MACD histogram opposes direction

**Minimum confidence thresholds:**
- **TRADEABLE**: 80+ (requires all mandatory conditions)
- **AVOID**: <80 (do not trade)

### 11. MARKET CONDITION BIAS
**Prioritize these setups:**
- **LONG in BULLISH 4H market**: Focus on moderate pullbacks (RSI 32-42, price within lower 30% of BB)
- **SHORT in BEARISH 4H market**: Focus on moderate rallies (RSI 58-68, price within upper 30% of BB)
- **AVOID** trading when 4H indicators show extreme readings
- **CONFIRM**: 4H RSI should be in moderate range (45-55) - not at extremes

### 12. RISK MANAGEMENT ENHANCEMENTS
**Implement these risk rules:**
- **Maximum position size**: 2% of portfolio per trade
- **Maximum daily loss**: 5% of portfolio
- **Stop loss placement**: Always below/above nearest swing point + 0.15% buffer
- **Take profit**: Set at 2.0% minimum, trail stop after 1.5% gain
