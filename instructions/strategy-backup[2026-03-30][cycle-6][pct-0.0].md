...
- **OPTIMAL SL distance**: 1.1-1.4% (target 1.25%)
- **MINIMUM SL distance**: 1.0% (tight stops get hit by noise)
- **MAXIMUM SL distance**: 1.6% (wide stops increase risk per trade)
- **REJECT** trades with SL distance < 1.0% or > 1.6%
- **ANALYSIS**: Losing trades had SL distances of 0.89% (too tight), 1.04% (borderline), and 1.46% (acceptable)

### 6. TARGET PROFIT DISTANCE OPTIMIZATION (UPDATED)
**Use these TP distance guidelines:**
- **OPTIMAL TP distance**: 2.2-2.8% (target 2.5%)
- **MINIMUM R:R ratio**: 1:2.0 (prefer 1:2.2+)
- **MAXIMUM R:R ratio**: 1:3.5 (avoid unrealistic targets)
- **AVOID** excessive R:R > 1:3.5 (indicates unrealistic targets)
- **ANALYSIS**: Losing trades had R:R ratios of 3.43, 2.16, and 1.77 - the 3.43 ratio was unrealistic
...
6. ✅ SL distance 1.1-1.4%
7. ✅ R:R ratio between 1:2.0 and 1:3.5
...
- **Maximum position size**: 1.0% of portfolio per trade (reduced from 1.5%)
- **Maximum daily loss**: 3% of portfolio (reduced from 4%)
- **Stop loss placement**: Always below/above nearest swing point + 0.25% buffer
- **Take profit**: Set at 2.5% minimum, trail stop after 2.0% gain
...
- **High volatility (ATR > 2%)**: Use wider SL (1.3-1.6%) and TP (2.8-3.5%)
- **Normal volatility (ATR 1-2%)**: Use standard SL (1.1-1.4%) and TP (2.2-2.8%)
- **Low volatility (ATR < 1%)**: Use tighter SL (1.0-1.2%) and TP (2.0-2.5%)
