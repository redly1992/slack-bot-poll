<!-- AI-IMPROVED: 2026-03-30 | Cycle 6 -->
...
## 📊 LEARNED RULES (Cycle 6)

### 1. BOLLINGER BAND POSITION: AVOID EXTREME MOMENTUM (>110% or <-10%)
**ANALYSIS**: 4 of 6 winning LONG trades had bb_pos >100% (110.0%, 104.7%, 100.8%, 107.2%), but 3 of 11 losing trades also had >110% (112.0%, 120.9%). The single winning SHORT had 5.7% bb_pos, while losing SHORT trades had extremes (-29.5%).
- **REQUIRE FOR LONG**: bb_pos 80-110% (strong momentum acceptable)
- **REQUIRE FOR SHORT**: bb_pos 0-20% (strong momentum acceptable)
- **REJECT LONG**: If bb_pos > 115% (extreme exhaustion)
- **REJECT SHORT**: If bb_pos < -15% (extreme breakdown)
- **RATIONALE**: Strong momentum (up to 110%) can work for LONG, but extremes (>115%) fail.

### 2. 15m EMA ALIGNMENT: MUST MATCH DIRECTION (CONFIRMED)
...
### 3. RSI(15m) RANGES: WINNERS SHOW MODERATE MOMENTUM
...
- **REQUIRE FOR LONG**: RSI(15m) 55-77 (moderate to strong momentum)
- **REQUIRE FOR SHORT**: RSI(15m) 25-45 (moderate to strong momentum)
...
### 4. 4H RSI: CRITICAL REVERSAL FILTER (CONFIRMED)
...
- **REJECT LONG**: If 4H RSI > 78 (high reversal risk)
- **REJECT SHORT**: If 4H RSI < 22 (high reversal risk)
- **PREFER**: 4H RSI 50-75 for LONG, 25-50 for SHORT
- **RATIONALE**: Extreme 4H RSI significantly increases reversal probability.

### 5. MACD HISTOGRAM: MUST NOT STRONGLY OPPOSE DIRECTION
...
- **REQUIRE FOR LONG**: MACD histogram MUST be > -60 (not strongly opposing)
- **REQUIRE FOR SHORT**: MACD histogram MUST be < 60 (not strongly opposing)
- **PREFER FOR LONG**: MACD histogram > -20 (positive or neutral momentum)
- **PREFER FOR SHORT**: MACD histogram < 20 (negative or neutral momentum)
- **RATIONALE**: MACD should not strongly oppose trade direction.

### 6. STOP LOSS DISTANCE: 2.0% IS OPTIMAL (CONFIRMED)
...
### 7. TAKE PROFIT DISTANCE: 4.0% BETTER THAN 6.0%
**ANALYSIS**: 5 of 6 winning trades hit TP at ~3.0-4.0% (actual P&L: 1.33-4.38%). Only 1 win had room for 6.0% TP. With 2.0% SL, 4.0% TP gives 1:2 R:R.
- **STANDARD**: TP distance 4.0% for all trades
- **RANGE**: 3.8% to 4.2% acceptable
- **RATIONALE**: 4.0% TP is more achievable based on actual winning trade extensions.

### 8. RISK-REWARD RATIO: 1:2 IS REALISTIC
**ANALYSIS**: Winning trades had actual R:R ratio 1:0.67-1:2.19 (based on actual P&L). With 4.0% TP and 2.0% SL, R:R becomes 1:2.
- **TARGET**: R:R ratio 1:2 for all trades
- **MINIMUM**: R:R ratio 1:1.5 required
- **AVOID**: R:R ratio < 1:1.5 (negative expectancy)
- **RATIONALE**: 1:2 R:R is realistic based on actual winning trade performance.

### 9. MARKET DIRECTION: TREND-FOLLOWING DOMINATES (CONFIRMED)
...
### 10. EXIT TYPE: WINNERS ARE TIME EXITS, LOSERS ARE SL HITS
**ANALYSIS**: 5 of 6 winning trades were TIME exits (83%). 7 of 11 losing trades were SL hits (64%).
...
### 11. CONFIDENCE LEVEL: NO CLEAR SEPARATOR
**ANALYSIS**: Winning trades had confidence 65-72 (avg 67). Losing trades had confidence 65-75 (avg 68). Confidence scores did not predict outcomes.
- **FOCUS**: On checklist conditions, not confidence score
- **SCORING**: Use confidence to reflect checklist compliance, not prediction

### 12. WINNING PATTERN: MODERATE MOMENTUM IN TREND DIRECTION
**ANALYSIS**: Winning trades consistently showed:
1. Price at moderate-to-strong Bollinger Band position (50-110% for LONG, 0-20% for SHORT)
2. RSI in moderate momentum zone (55-77 for LONG, 25-45 for SHORT)
3. 15m EMA alignment matching direction
4. MACD histogram not strongly opposing direction (> -60 for LONG, < 60 for SHORT)
5. Trend-following only (no counter-trend)
6. 4H RSI not extreme (<78 for LONG, >22 for SHORT)

### 13. LOSING PATTERN AVOIDANCE:
**Based on trade analysis, AVOID:**
1. ❌ bb_pos extreme exhaustion (>115% for LONG, <-15% for SHORT)
2. ❌ RSI(15m) extremes (>78 for LONG, <20 for SHORT)
3. ❌ 4H RSI extreme (>78 for LONG, <22 for SHORT)
4. ❌ Counter-trend trades
5. ❌ MACD histogram strongly opposing (< -60 for LONG, > 60 for SHORT)
6. ❌ Weak momentum (RSI(15m) <50 for LONG, >50 for SHORT)

### 14. ENTRY QUALITY CHECKLIST:
**REQUIRE for entry:**
1. ✅ 15m EMA alignment MATCHES direction (bullish for LONG, bearish for SHORT)
2. ✅ bb_pos in moderate range (80-110% for LONG, 0-20% for SHORT)
3. ✅ RSI(15m) in moderate zone (55-77 for LONG, 25-45 for SHORT)
4. ✅ MACD histogram not strongly opposing (> -60 for LONG, < 60 for SHORT)
5. ✅ 4H RSI not extreme (<78 for LONG, >22 for SHORT)
6. ✅ Market direction aligned (trend-following only)
7. ✅ SL distance 2.0% (±0.05%)
8. ✅ TP distance 4.0% (±0.05%)
9. ✅ R:R ratio 1:2 (±0.2)
10. ✅ Minimum 3 confirming indicators from above

### 15. CRITICAL CONDITIONS (IMMEDIATE REJECTION):
1. ❌ 15m EMA alignment opposes trade direction
2. ❌ bb_pos extreme exhaustion (LONG: >115%, SHORT: <-15%)
3. ❌ RSI(15m) extremes (LONG: >78 or <50, SHORT: <20 or >50)
4. ❌ 4H RSI extreme (LONG: >78, SHORT: <22)
5. ❌ MACD histogram strongly opposes (< -60 for LONG, > 60 for SHORT)
6. ❌ Counter-trend trade

### 16. PARAMETER SUMMARY:
- **15m EMA alignment**: MUST match trade direction
- **bb_pos LONG**: 80-110% required (strong momentum acceptable)
- **bb_pos SHORT**: 0-20% required (strong momentum acceptable)
- **RSI(15m) LONG**: 55-77 required (moderate to strong momentum)
- **RSI(15m) SHORT**: 25-45 required (moderate to strong momentum)
- **4H RSI LONG**: MUST be <78 (50-75 optimal)
- **4H RSI SHORT**: MUST be >22 (25-50 optimal)
- **MACD hist LONG**: > -60 required (not strongly opposing)
- **MACD hist SHORT**: < 60 required (not strongly opposing)
...
- **Minimum confirmations**: 3+ indicators from checklist
...
### 18. DO NOT TRADE IF:
1. bb_pos shows extreme exhaustion (>115% for LONG, <-15% for SHORT)
2. RSI(15m) is extreme (>78 for LONG, <20 for SHORT)
3. 15m EMA alignment opposes trade direction
4. 4H RSI is extreme (>78 for LONG, <22 for SHORT)
5. MACD strongly opposes direction (< -60 for LONG, > 60 for SHORT)
...
### 20. KEY IMPROVEMENTS FROM CYCLE 5:
1. **Adjusted bb_pos ranges**: 80-110% for LONG (was 80-105%), 0-20% for SHORT (same)
2. **Adjusted RSI(15m) ranges**: 55-77 for LONG (was 60-75), 25-45 for SHORT (was 25-40)
3. **Adjusted 4H RSI filter**: Reject >78 for LONG (was >75), reject <22 for SHORT (was <25)
4. **MACD requirement adjusted**: Not strongly opposing (> -60 for LONG, < 60 for SHORT)
5. **TP distance decreased**: 4.0% (was 6.0%) for realistic R:R
6. **R:R target**: 1:2 (was 1:3) based on actual performance
7. **Same minimum confirmations**: 3+ indicators
8. **Banned counter-trend trades**: Trend-following only (same)

### 21. REMEMBER:
This is a **MODERATE MOMENTUM TREND-FOLLOWING** strategy. Enter when price shows moderate-to-strong momentum (not extreme exhaustion) in the direction of the larger trend, with multiple indicators confirming continuation. Avoid extreme zones, counter-trend setups, and any trade without momentum confirmation. Use 2.0% SL and 4.0% TP for realistic 1:2 risk-reward.