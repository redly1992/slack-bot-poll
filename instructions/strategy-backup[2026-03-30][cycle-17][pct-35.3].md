...
### 1. LONG-ONLY IN BULLISH MARKETS (CONFIRMED)
**ANALYSIS**: 5/5 winning trades were LONG in BULLISH markets. 0/2 SHORT trades were winners. LONG in BULLISH has 100% win rate in sample.
...
- **RATIONALE**: Trend-following LONG in BULLISH markets has perfect win rate
...
### 1. STRICT DIRECTIONAL FILTERING (IMPROVED)
**ANALYSIS**: All 6 wins were LONG in BULLISH markets. 0/2 SHORT trades won. 7 LONG losses occurred when conditions weren't optimal.
- **REQUIRE**: Only LONG in BULLISH 4H markets (price > EMA21 > EMA50)
- **IMMEDIATE REJECTION**: SHORT signals in ANY market condition
- **IMMEDIATE REJECTION**: LONG in BEARISH or NEUTRAL 4H markets
- **ADDITIONAL**: Daily timeframe should confirm or be neutral (not bearish)
- **RATIONALE**: 100% win rate for LONG in BULLISH vs 0% for SHORT

### 2. OPTIMAL STOP LOSS: 2.0% EXACT (CONFIRMED)
**ANALYSIS**: Winning trades used 1.96-2.06% SL (avg 2.00%). Losing trades had more variance.
- **REQUIRE**: SL distance MUST be 2.0% exactly (1.98-2.02% range)
- **CALCULATION**: Entry price × 0.98 for LONG positions
- **NO DEVIATION**: Never use arbitrary SL placement based on structure alone
- **RATIONALE**: Standardized 2.0% SL provides consistent risk management

### 3. BOLLINGER BAND POSITION: 90-105% OPTIMAL (REFINED)
**ANALYSIS**: Winning trades had bb_pos 50.8-110.0% but optimal range is narrower.
- **REQUIRE**: bb_pos 80-110% for entry consideration
- **OPTIMAL**: 90-105% for highest probability entries
- **REJECT**: bb_pos < 80% (weak momentum) OR > 110% (exhausted move)
- **PREFER**: 95-102% for strongest momentum continuation setups
- **RATIONALE**: Moderate overextension indicates healthy momentum, not exhaustion

### 4. 15m RSI: 65-72 SWEET SPOT (REFINED)
**ANALYSIS**: Winning trades had RSI(15m) 55.9-76.3 but optimal is narrower.
- **REQUIRE**: RSI(15m) 60-76 for entry
- **OPTIMAL**: 65-72 for best momentum continuation
- **REJECT**: RSI(15m) < 60 (weak momentum) OR > 76 (extreme overbought)
- **CAUTION**: Reduce position size 50% if RSI(15m) > 74
- **RATIONALE**: Moderate overbought indicates continuation, not reversal

### 5. 4H RSI: 60-70 OPTIMAL ZONE (REFINED)
**ANALYSIS**: Winning trades had 4H RSI 57.7-76.5 but losses clustered at extremes.
- **REQUIRE**: 4H RSI 58-73 for entry
- **OPTIMAL**: 60-70 for strongest uptrend continuation
- **REJECT**: 4H RSI < 58 (weak trend) OR > 73 (high reversal risk)
- **REDUCE SIZE 50%**: If 4H RSI 71-73 (caution zone)
- **RATIONALE**: 4H RSI extremes significantly increase pullback probability

### 6. MACD HISTOGRAM STRENGTH MATTERS (NEW)
**ANALYSIS**: Winning trades had MACD histogram 54.34-159.02. Losing trades had weaker values.
- **REQUIRE**: MACD histogram > 0 (positive momentum)
- **PREFER**: MACD histogram > 50 for stronger setups
- **OPTIMAL**: MACD histogram > 75 for highest conviction
- **REJECT**: MACD histogram < 0 (negative momentum)
- **WARNING**: MACD histogram < 20 indicates weak momentum
- **RATIONALE**: Stronger positive histogram indicates healthier momentum continuation

### 7. 15m EMA ALIGNMENT & GAP (ENHANCED)
**ANALYSIS**: All winning trades had bullish EMA alignment with meaningful gaps.
- **REQUIRE**: 15m EMA alignment MUST be bullish (ema_fast > ema_slow)
- **ADDITIONAL**: EMA gap should be > 0.15% of price (meaningful separation)
- **PREFER**: EMA gap 0.2-0.5% for strongest alignment
- **REJECT**: EMA gap < 0.1% (tangled/weak alignment)
- **RATIONALE**: Clear EMA separation confirms strong short-term momentum

### 8. EXIT STRATEGY: TRAILING STOPS BEAT FIXED TP (CONFIRMED)
**ANALYSIS**: 5/6 wins were TIME exits with trailing stops. Fixed TP often left profit on table.
- **PRIMARY EXIT**: Time-based with trailing stop activation
- **TRAILING STOP**: Activate at 1.5% unrealized gain, trail at 0.75% below current
- **PARTIAL PROFIT**: Take 25% at 2.5% gain, trail rest
- **AVOID**: Fixed TP at specific % (winners often exceed 4% before TIME exit)
- **RATIONALE**: Letting winners run captures more profit than fixed TP

### 9. RISK-REWARD: MINIMUM 1:2.5 PREFERRED (ENHANCED)
**ANALYSIS**: Winning trades had RR ratios 1.03-4.02 (avg 2.55).
- **REQUIRE**: Minimum 1:2 risk-reward ratio
- **PREFER**: 1:2.5 or better for higher quality setups
- **CALCULATE**: Based on nearest significant resistance level
- **AVOID**: RR < 1:2 even if all other conditions met
- **RATIONALE**: Favorable RR compensates for imperfect win rate

### 10. VOLATILITY FILTER: 4H CANDLE RANGE (NEW)
**ANALYSIS**: Losing trades often occurred during high volatility periods.
- **CHECK**: Recent 4H candle range as % of price
- **REJECT**: If 4H candle range > 3.5% (excessive volatility)
- **PREFER**: 4H candle range 1.5-2.5% (healthy trend movement)
- **CAUTION**: Reduce size 50% if range 3.0-3.5%
- **RATIONALE**: Excessive volatility increases stop loss hit probability

### 11. DAILY TIMEFRAME CONFIRMATION (ENHANCED)
**ANALYSIS**: Multi-timeframe alignment improves success.
- **REQUIRE**: Daily timeframe bullish OR neutral (not bearish)
- **PREFER**: Daily also bullish (price > Daily EMA21)
- **BONUS**: Daily EMA alignment bullish (fast > slow)
- **REDUCE SIZE 50%**: If Daily neutral (not confirming)
- **REJECT**: If Daily bearish while 4H bullish (major conflict)
- **RATIONALE**: Higher timeframe alignment increases setup quality

### 12. RECENT PRICE ACTION CONTEXT (NEW)
**ANALYSIS**: Winning trades often followed consolidation periods.
- **PREFER**: 2-4 candles of sideways consolidation before entry
- **AVOID**: Entry immediately after large impulsive move (>3% in 4H)
- **CHECK**: Recent 4H candles for healthy pullback patterns
- **OPTIMAL**: Consolidation range < 1.5% before continuation
- **RATIONALE**: Consolidation provides better risk-reward entry

### 13. POSITION SIZING MATRIX (ENHANCED)
**Based on confluence strength:**
- **BASE SIZE** (100%): All 6 core conditions met, 4H RSI 60-70, bb_pos 90-105%
- **REDUCE 50%**: If 4H RSI 71-73 OR Daily neutral OR RSI(15m) > 74
- **REDUCE 75%**: If 2+ caution conditions present
- **INCREASE 25%**: If ALL optimal: 4H RSI 62-68, bb_pos 95-102%, MACD hist > 75, Daily bullish
- **MAX 1 TRADE/DAY**: Prevent overtrading

### 14. ENTRY TIMING PRECISION (NEW)
**ANALYSIS**: Better timing improves win rate.
- **WAIT**: For 15m candle close confirming setup
- **AVOID**: Entering mid-candle during volatile moves
- **PREFER**: Entry during lower volatility periods (London/NY overlap)
- **CHECK**: 15m volume - increasing on breakout confirmation
- **RATIONALE**: Precise timing reduces slippage and false entries

### 15. CONFLUENCE SCORING SYSTEM (NEW)
**Score each condition (must score ≥8 to enter):**

1. 4H Market BULLISH (price > EMA21 > EMA50): +2 points
2. bb_pos 90-105%: +2 points (80-89% or 106-110%: +1 point)
3. RSI(