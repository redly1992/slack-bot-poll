...
- **REQUIRE FOR LONG**: bb_pos between 20% and 45% (optimal: 30-40%)
- **REQUIRE FOR SHORT**: bb_pos between 55% and 80% (optimal: 60-70%)
- **REJECT**: bb_pos < 15% or > 85% (extreme - DO NOT TRADE)
...
- **REQUIRE FOR LONG**: Stochastic %K between 30 and 50 (optimal: 35-45)
- **REQUIRE FOR SHORT**: Stochastic %K between 50 and 70 (optimal: 55-65)
- **REJECT**: Stochastic %K < 25 or > 75 (extreme - DO NOT TRADE)
...
- **REQUIRE FOR LONG**: MACD hist > -80 and showing improvement (less negative or turning positive)
- **REQUIRE FOR SHORT**: MACD hist < 80 and showing deterioration (less positive or turning negative)
- **REJECT**: MACD hist < -120 or > 120 (extreme momentum - DO NOT TRADE)
...
- **OPTIMAL SL distance**: 2.5-3.0% (adjusted from 2.2-2.8%)
- **MINIMUM SL distance**: 2.2% (increased from 2.0%)
- **MAXIMUM SL distance**: 3.5%
- **REQUIRE**: SL distance ≥ 2.2% for confidence ≥ 70
- **RATIONALE**: Tighter stops (<2.2%) get stopped by noise in volatile crypto markets
...
- **MINIMUM R:R ratio**: 1:2.2 (increased from 1:2.0)
- **PREFER R:R ratio**: 1:2.5 to 1:3.0
- **REQUIRE**: R:R ≥ 1:2.2 for confidence ≥ 70
...
- **MINIMUM EXECUTION CONFIDENCE**: 90 (increased from 85)
...
3. bb_pos in optimal range (20-45% LONG, 55-80% SHORT)
4. RSI(15m) in optimal range (35-45 LONG, 55-65 SHORT)
5. MACD hist in optimal range (> -80 LONG, < 80 SHORT) and improving
6. Stochastic %K in optimal range (30-50 LONG, 50-70 SHORT) and turning
7. SL distance 2.2-3.5%
8. R:R ratio ≥ 1:2.2
- **MINIMUM**: 7 out of 8 factors must confirm for confidence ≥ 90
- **PREFER**: 8 out of 8 factors confirm for confidence ≥ 95
- **REJECT**: Less than 7 factors confirm (confidence ≤ 70)

### 10. ENTRY CHECKLIST - STRICT VERSION (CYCLE 17)
**REQUIRE ALL of these for execution (confidence ≥ 90):**

**For ALL entries:**
1. ✅ 4H trend clearly in trade direction (price above/below EMAs, RSI confirming)
2. ✅ 15m EMA alignment MATCHING direction (bullish for LONG, bearish for SHORT) - MANDATORY
3. ✅ bb_pos in optimal range (20-45% for LONG, 55-80% for SHORT)
4. ✅ RSI(15m) in optimal range (35-45 for LONG, 55-65 for SHORT)
5. ✅ MACD hist in optimal range (> -80 for LONG, < 80 for SHORT) and improving
6. ✅ Stochastic %K in optimal range (30-50 for LONG, 50-70 for SHORT) and turning
7. ✅ SL distance 2.2-3.5%
...
- **High volatility (ATR > 3.0%)**: Use wider SL (2.8-3.5%), larger TP (5.5-6.0%)
- **Normal volatility (ATR 2.0-3.0%)**: Use standard SL (2.5-3.0%), TP (5.0-5.5%)
- **Low volatility (ATR < 2.0%)**: Use tighter SL (2.2-2.7%), smaller TP (4.5-5.0%)
- **Check 4H ATR** relative to price for context

### 13. TIME-BASED EXIT RULE (PRECISE)
**Exit if trade doesn't perform:**
- **Trend-following trades**: Exit if no profit within 3 candles (reduced from 4)
- **Scale out**: Take 50% profit at 3.0%, trail stop on remainder
...
- **Position size**: 0.10-0.20% of portfolio per trade (reduced from 0.15-0.25%)
- **Maximum daily loss**: 0.4% of portfolio (reduced from 0.5%)
- **Stop loss placement**: Below/above structure + 1.5% minimum buffer
- **Take profit**: Primary at 5.0%, secondary at 3.0% with trail
- **Maximum consecutive losses**: 1 trade → pause trading for 48 hours
...
- **AVOID**: Extreme Stochastic (<30 or >70) (all 3 losses had extreme values)
- **AVOID**: Extreme MACD hist (<-80 or >80) (2 of 3 losses had this)
- **AVOID**: SL distance < 2.2% (all 3 losses had this)
- **AVOID**: Confidence < 90 (all 3 losses were ≤85 confidence)

### 18. OPTIMAL PARAMETER RANGES (PRECISE)
**Based on backtest analysis and adjustments:**
- **SL distance**: 2.7% optimal (range: 2.5-3.0%)
- **TP distance**: 5.5% optimal (range: 5.0-6.0%)
- **R:R ratio**: 1:2.5 optimal (range: 1:2.2-1:3.0)
- **bb_pos LONG**: 35% optimal (range: 30-40%)
- **bb_pos SHORT**: 65% optimal (range: 60-70%)
...
- **MACD hist LONG**: > -40 optimal (range: > -80)
- **MACD hist SHORT**: < 40 optimal (range: < 80)
...
2. ✅ bb_pos in optimal range (20-45% LONG, 55-80% SHORT)
3. ✅ RSI(15m) in optimal range (35-45 LONG, 55-65 SHORT)
4. ✅ Stochastic in optimal range (30-50 LONG, 50-70 SHORT) and turning
5. ✅ MACD hist in optimal range (> -80 LONG, < 80 SHORT) and improving
6. ✅ SL distance 2.2-3.5%
...
- bb_pos: 30-40% (moderate oversold, not extreme)
- RSI(15m): 38-42 (moderate oversold)
- Stochastic: 35-45 and rising
- MACD hist: > -40 and improving
- SL distance: ≥ 2.5%
- TP distance: ≥ 5.0%
- R:R ratio: ≥ 1:2.2
- Confidence: ≥ 90
...
- bb_pos: 60-70% (moderate overbought, not extreme)
- RSI(15m): 58-62 (moderate overbought)
- Stochastic: 55-65 and falling
- MACD hist: < 40 and deteriorating
- SL distance: ≥ 2.5%
- TP distance: ≥ 5.0%
- R:R ratio: ≥ 1:2.2
- Confidence: ≥ 90

### 21. REMOVED/REPLACED RULES FROM CYCLE 16
**Replaced these Cycle 16 rules:**
- ❌ "SL distance: 2.2-2.8%" → **REPLACED WITH** "SL distance: 2.5-3.0%"
- ❌ "TP distance: 4.5-5.5%" → **REPLACED WITH** "TP distance: 5.0-6.0%"
- ❌ "R:R ratio ≥ 1:2.0" → **REPLACED WITH** "R:R ratio ≥ 1:2.2"
- ❌ "bb_pos: 15-40% LONG, 60-85% SHORT" → **REPLACED WITH** "bb_pos: 20-45% LONG, 55-80% SHORT"
- ❌ "RSI(15m): 32-42 LONG, 58-68 SHORT" → **REPLACED WITH** "RSI(15m): 35-45 LONG, 55-65 SHORT"
- ❌ "Stochastic: 25-45 LONG, 55-75 SHORT" → **REPLACED WITH** "Stochastic: 30-50 LONG, 50-70 SHORT"
- ❌ "MACD hist: > -100 LONG, < 100 SHORT" → **REPLACED WITH** "MACD hist: > -80 LONG, < 80 SHORT"
- ❌ "Confidence threshold: 85+" → **REPLACED WITH** "Confidence threshold: 90+"

**Enhanced these Cycle 16 rules:**
...
### 22. NEW RULES FOR CYCLE 17 (BASED ON 3 LOSSES ANALYSIS)
**Added these new rules:**

1. **Stricter bb_pos Ranges (20-45% LONG, 55-80% SHORT)**
...
2. **Tighter MACD Histogram Momentum Requirement**
   ...
   - Require MACD hist > -80 for LONG, < 80 for SHORT and showing improvement

3. **Higher Confidence Threshold (90+)**
...
**Based on analysis of 3 losing trades:**
- **SL distance**: 2.5-3.0% (increase from 2.2-2.8%)
- **TP distance**: 5.0-6.0% (increase from 4.5-5.5%)
- **R:R ratio**: 1:2.2 to 1:3.0 (minimum increased)
- **Confidence threshold**: 90+ to execute (increased from 85)
- **Position size**: 0.10-0.20% of portfolio per trade (reduced)
...
3. **Check bb_pos** (must be 20-45% LONG, 55-80% SHORT - if not, STOP)
4. **Check RSI(15m)** (must be 35-45 LONG, 55-65 SHORT - if not, STOP)
5. **Check Stochastic** (must be 30-50 LONG, 50-70 SHORT and turning - if not, STOP)
6. **Check MACD hist** (must be > -80 LONG, < 80 SHORT and improving - if not, STOP)
7. **Calculate SL/TP distances** (must be in optimal ranges)
8. **Calculate R:R ratio** (must be ≥ 1:2.2)
9. **Calculate confidence** (must be ≥ 90)
