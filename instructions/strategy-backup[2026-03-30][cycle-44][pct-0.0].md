<!-- AI-IMPROVED: 2026-03-30 | Cycle 43 -->
...
- **REQUIRE FOR LONG**: RSI(15m) MUST be between 40 and 55
- **REQUIRE FOR SHORT**: RSI(15m) MUST be between 45 and 60
- **AVOID**: RSI(15m) < 40 for LONG (too oversold, indicates breakdown)
- **AVOID**: RSI(15m) > 60 for SHORT (too overbought, indicates breakout)
- **RATIONALE**: Moderate RSI levels indicate healthy pullback/rally, not exhaustion or trend break.

### 4. STOP LOSS DISTANCE: 2.2-2.8% (ADEQUATE BUFFER)
**ANALYSIS**: Losing trades had SL distances: 2.04%, 1.75%, 2.18%. The 1.75% was too tight and got hit.
- **OPTIMAL RANGE**: SL distance 2.2-2.8% (2.5% optimal)
- **MINIMUM**: 2.2% (adequate buffer for crypto volatility)
- **MAXIMUM**: 2.8% (risk management limit)
- **AVOID**: SL distance < 2.2% (too tight, prone to stop runs)
- **RATIONALE**: Wider stops (2.2-2.8%) allow for normal volatility while maintaining good R:R.

### 5. TAKE PROFIT DISTANCE: 2.5-3.5% (ACHIEVABLE TARGETS)
**ANALYSIS**: Losing trades had TP distances: 2.29%, 2.03%, 2.15%. Need more ambitious but achievable targets.
- **OPTIMAL RANGE**: TP distance 2.5-3.5% (3.0% optimal)
- **MINIMUM**: 2.5% (reasonable reward)
- **MAXIMUM**: 3.5% (achievable)
- **RATIONALE**: Conservative TP targets (2.5-3.5%) are frequently hit in normal market moves.

### 6. RISK-REWARD RATIO: 1:1.2 to 1:1.5 (POSITIVE EXPECTANCY)
**ANALYSIS**: Losing trades had R:R ratios: 1.01, 0.96, 0.99. Need positive expectancy.
- **OPTIMAL RANGE**: R:R ratio 1:1.2 to 1:1.5 (1:1.3 optimal)
- **MINIMUM**: 1:1.2 (positive expectancy after fees)
- **MAXIMUM**: 1:1.5 (achievable with good win rate)
- **RATIONALE**: Realistic R:R ratios (1:1.2-1:1.5) have higher expectancy than aggressive ratios.

### 7. CONFIDENCE THRESHOLD: 90+ TO EXECUTE (WITH STRICT FILTERS)
**ANALYSIS**: Losing trades had confidence of 45, 92, 92. Confidence alone is not enough.
- **MINIMUM EXECUTION**: Confidence 90+ WITH all technical filters passing
...
### 9. STOCHASTIC: AVOID EXTREMES (<20 or >80)
**ANALYSIS**: Losing trades had Stochastic extremes: 11.6, 94.9, 3.8.
- **REQUIRE FOR LONG**: Stochastic %K MUST be between 25 and 60
- **REQUIRE FOR SHORT**: Stochastic %K MUST be between 40 and 75
- **AVOID**: Stochastic %K < 25 or > 75 (extreme readings)
...
4. ✅ RSI(15m) in strict range (40-55 for LONG, 45-60 for SHORT)
5. ✅ Stochastic %K in optimal range (25-60 for LONG, 40-75 for SHORT)
6. ✅ MACD histogram showing momentum turning toward trade direction
7. ✅ SL distance 2.2-2.8% from entry
8. ✅ TP distance 2.5-3.5% from entry
9. ✅ R:R ratio 1:1.2 to 1:1.5
...
4. ✅ RSI(15m) in optimal range (40-55 LONG, 45-60 SHORT)
5. ✅ Stochastic %K in optimal range (25-60 LONG, 40-75 SHORT)
6. ✅ MACD histogram turning toward trade direction
7. ✅ SL distance 2.2-2.8%
8. ✅ TP distance 2.5-3.5%
9. ✅ R:R ratio 1:1.2-1:1.5
10. ✅ Confidence ≥ 90
...
3. ❌ RSI extremes (<40 for LONG, >60 for SHORT)
4. ❌ Stochastic extremes (<25 or >75)
5. ❌ SL distance < 2.2% (too tight, prone to stop runs)
6. ❌ MACD histogram strongly opposing trade direction

### 14. PARAMETER SUMMARY (CYCLE 43):
...
- **RSI(15m) LONG**: 40-55 optimal
- **RSI(15m) SHORT**: 45-60 optimal
- **Stochastic LONG**: 25-60 optimal
- **Stochastic SHORT**: 40-75 optimal
- **MACD hist**: Must be turning toward trade direction
- **SL distance**: 2.5% optimal (2.2-2.8% range)
- **TP distance**: 3.0% optimal (2.5-3.5% range)
- **R:R ratio**: 1:1.3 optimal (1:1.2-1:1.5 range)
- **Confidence threshold**: 90+ minimum WITH all filters
...
8. **Calculate SL/TP distances** (must be in optimal ranges: SL 2.2-2.8%, TP 2.5-3.5%)
9. **Calculate R:R ratio** (must be 1:1.2-1:1.5)
10. **Calculate confidence** (must be ≥90)
11. **If ALL conditions met** → Execute
12. **If ANY condition not met** → Do not execute

### 16. STRATEGY ADJUSTMENTS FROM CYCLE 42:
1. **MAINTAIN 15m EMA alignment** as FIRST CHECK (non-negotiable) - CONFIRMED CRITICAL
2. **TIGHTEN bb_pos range** to 20-80% (from 10-90%) - avoid extremes more strictly
3. **ADJUST RSI ranges** (40-55 LONG, 45-60 SHORT) - more conservative boundaries
4. **INCREASE SL distance** to 2.2-2.8% (from 2.0-2.5%) - better buffer
5. **INCREASE TP distance** to 2.5-3.5% (from 2.0-2.5%) - more ambitious but achievable
6. **INCREASE R:R ratio** to 1:1.2-1:1.5 (from 1:1.0-1:1.2) - positive expectancy
7. **INCREASE confidence threshold** to 90+ (from 85+) - stricter quality control
8. **REQUIRE 5/5 indicators optimal** (from 4/5) - stricter consensus
9. **TIGHTEN Stochastic ranges** (25-60 LONG, 40-75 SHORT) - avoid extremes
10. **REQUIRE MACD turning momentum** - stricter filter
...
3. **SL distance < 2.2% is too tight** - 1.75% stop got hit
4. **Confidence alone is insufficient** - need strict technical filters
5. **All indicators must align** - 5/5 optimal required
6. **Avoid indicator extremes** - RSI, Stochastic, bb_pos all showed extremes in losses
...
3. **Use SL distance 2.2-2.8%** - adequate buffer for crypto volatility
4. **Use TP distance 2.5-3.5%** - realistic, achievable targets
5. **Aim for R:R ratio 1:1.3** - positive expectancy
6. **Require confidence 90+ WITH all filters** - combined approach
...
### 19. UPDATED EXECUTION CHECKLIST (CYCLE 43):
...
### 20. TRADE FILTERING PROTOCOL (CYCLE 43):
...
6. **Sixth Filter**: Check SL distance - REJECT if outside 2.2-2.8%
7. **Seventh Filter**: Check TP distance - REJECT if outside 2.5-3.5%
8. **Eighth Filter**: Check R:R ratio - REJECT if outside 1:1.2-1:1.5
9. **Final Check**: Confidence ≥90 - REJECT if below
