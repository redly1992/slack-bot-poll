<!-- AI-IMPROVED: 2026-03-30 | Cycle 42 -->
...
## 📊 LEARNED RULES (Cycle 42)

### 1. 15m EMA ALIGNMENT IS ABSOLUTELY CRITICAL - 100% FAILURE WHEN WRONG
...
- **REQUIRE**: bb_pos MUST be between 10% and 90%
- **OPTIMAL LONG**: bb_pos 20-40% (healthy pullback, not breakdown)
- **OPTIMAL SHORT**: bb_pos 60-80% (healthy rally, not breakout)
- **AVOID**: bb_pos < 10% (too oversold, trend may be broken)
- **AVOID**: bb_pos > 90% (too overbought, trend may be broken)
...
### 4. STOP LOSS DISTANCE: 2.0-2.5% (ADEQUATE BUFFER)
...
- **AVOID**: SL distance < 2.0% (too tight, prone to stop runs)
- **RATIONALE**: Wider stops (2.0-2.5%) allow for normal volatility while maintaining good R:R.

### 5. TAKE PROFIT DISTANCE: 2.0-2.5% (CONSERVATIVE TARGETS)
**ANALYSIS**: Losing trades had TP distances: 2.29%, 2.03%, 2.15%. Need consistent targets.
- **OPTIMAL RANGE**: TP distance 2.0-2.5% (2.2% optimal)
- **MINIMUM**: 2.0% (reasonable reward)
- **MAXIMUM**: 2.5% (achievable)
- **RATIONALE**: Conservative TP targets (2.0-2.5%) are frequently hit in normal market moves.

### 6. RISK-REWARD RATIO: 1:1.0 to 1:1.2 (REALISTIC)
**ANALYSIS**: Losing trades had R:R ratios: 1.01, 0.96, 0.99. Need slightly positive expectancy.
- **OPTIMAL RANGE**: R:R ratio 1:1.0 to 1:1.2 (1:1.1 optimal)
- **MINIMUM**: 1:1.0 (breakeven after fees)
- **MAXIMUM**: 1:1.2 (achievable with good win rate)
- **RATIONALE**: Realistic R:R ratios (1:1.0-1:1.2) have higher hit rates than aggressive ratios.
...
### 8. MACD MOMENTUM: MUST SUPPORT OR BE TURNING
**ANALYSIS**: Losing trades had MACD histograms opposing or weak momentum.
- **REQUIRE FOR LONG**: MACD histogram MUST be positive OR turning positive (less negative)
- **REQUIRE FOR SHORT**: MACD histogram MUST be negative OR turning negative (less positive)
- **AVOID**: MACD histogram strongly opposing trade direction
...
### 10. INDICATOR CONSENSUS: 4/5 IN OPTIMAL RANGES (STRICT)
**ANALYSIS**: Losing trades had mixed indicator signals with extremes.
- **REQUIRE**: At least 4 out of 5 indicators must be in optimal ranges
- **5 INDICATORS**: RSI, Stochastic, MACD, Bollinger Bands, EMA alignment
- **CRITICAL**: EMA alignment must ALWAYS match (non-negotiable)
- **RATIONALE**: Strong consensus across indicators increases probability of success.
...
2. ❌ bb_pos outside 10-90% range (all losses had extremes)
...
### 14. PARAMETER SUMMARY (CYCLE 42):
- **15m EMA alignment**: MUST match trade direction (NON-NEGOTIABLE - CHECK FIRST)
- **bb_pos range**: 10-90% required (avoid extremes)
...
- **MACD hist**: Must support or be turning toward trade direction
- **SL distance**: 2.2% optimal (2.0-2.5% range)
- **TP distance**: 2.2% optimal (2.0-2.5% range)
- **R:R ratio**: 1:1.1 optimal (1:1.0-1:1.2 range)
- **Confidence threshold**: 85+ minimum WITH all filters
- **Indicator consensus**: 4/5 must be optimal
...
4. **Check bb_pos** (MUST be 10-90%)
5. **Check RSI(15m)** (MUST be in optimal range)
6. **Check Stochastic** (MUST be in optimal range)
7. **Check MACD histogram** (MUST support or be turning toward trade direction)
8. **Calculate SL/TP distances** (must be in optimal ranges: both 2.0-2.5%)
9. **Calculate R:R ratio** (must be 1:1.0-1:1.2)
10. **Calculate confidence** (must be ≥85)
11. **If ALL conditions met** → Execute
12. **If ANY condition not met** → Do not execute

### 16. STRATEGY ADJUSTMENTS FROM CYCLE 41:
1. **MAINTAIN 15m EMA alignment** as FIRST CHECK (non-negotiable) - CONFIRMED CRITICAL
2. **WIDEN bb_pos range** to 10-90% (from 30-70%) - allow more flexibility while avoiding extremes
3. **ADJUST RSI ranges** (35-50 LONG, 50-65 SHORT) - more realistic boundaries
4. **MAINTAIN SL distance** 2.0-2.5% - confirmed adequate
5. **INCREASE TP distance** to 2.0-2.5% (from 1.8-2.2%) - more realistic targets
6. **ADJUST R:R ratio** to 1:1.0-1:1.2 (from 1:0.9-1:1.1) - slightly more ambitious but realistic
7. **ADJUST confidence threshold** to 85+ with strict filters (from 90+) - more practical
8. **REQUIRE 4/5 indicators optimal** (from 5/5) - more flexible while maintaining quality
9. **MAINTAIN Stochastic range limits** (20-80) - avoid extremes
10. **REQUIRE MACD momentum confirmation** - maintain filter
...
5. **Multiple confirmation needed** - at least 4/5 indicators must be optimal
...
### 19. UPDATED EXECUTION CHECKLIST (CYCLE 42):
...
### 20. TRADE FILTERING PROTOCOL (CYCLE 42):
1. **First Filter**: Check 15m EMA alignment - REJECT IMMEDIATELY if not matching
2. **Second Filter**: Check bb_pos - REJECT if outside 10-90%
3. **Third Filter**: Check RSI(15m) - REJECT if outside optimal ranges
4. **Fourth Filter**: Check Stochastic - REJECT if outside 20-80
5. **Fifth Filter**: Check MACD histogram - REJECT if strongly opposing trade direction
6. **Sixth Filter**: Check SL/TP distances - REJECT if outside 2.0-2.5% ranges
7. **Seventh Filter**: Check R:R ratio - REJECT if outside 1:1.0-1:1.2
8. **Final Check**: Confidence ≥85 - REJECT if below
