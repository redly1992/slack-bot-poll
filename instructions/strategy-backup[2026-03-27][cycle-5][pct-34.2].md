...
<!-- AI-IMPROVED: 2026-01-16 -->
<!-- Based on 243 trades: 83 wins / 160 losses (34.2% win rate) -->
<!-- DATA-DRIVEN UPDATE: 2026-01-16 - Detailed analysis of 30 sample trades (15 wins, 15 losses) -->
<!-- KEY INSIGHT: Positive P&L ($47.74) despite low win rate indicates large average win size -->
...
1. **Win Rate**: 34.2% (83 wins, 160 losses) - low win rate but positive P&L
2. **Total P&L**: $47.74 (positive despite low win rate)
3. **Critical Insight**: Average win size significantly larger than average loss size (estimated 2:1 ratio)
4. **Performance Patterns Identified**:
   ...
   - **MACD Patterns**: Extreme 4h MACD_hist (>400) often leads to losses
...
1. **MARKET CONDITION FILTER (CRITICAL)**:
   ...
   - ✅ **ALLOW**: SHORT in BULLISH markets ONLY when:
     ...
     - 4h MACD_hist is NEGATIVE or weakly positive (< 100)
     - 15m MACD_hist between -50 and +30
   - ⚠️ **CAUTION**: Trades in BEARISH markets - only take with strong confirmation
   - ❌ **AVOID**: LONG trades when 4h MACD_hist > 400 (exhaustion - consistent losing pattern)
   - ❌ **AVOID**: Trades when 15m EMA fast and slow are within 0.05% of each other (indecision)

2. **15m MACD HISTOGRAM THRESHOLDS (PRECISE)**:
   - **For LONG entries**: MACD_hist between -60 and +50 (optimal: -30 to +20)
   - **For SHORT entries**: MACD_hist between -50 and +40 (optimal: -40 to +20)
   - **EXTREME VALUES**: >100 or <-60 often lead to immediate reversals
...
3. **EMA ALIGNMENT RULES (MANDATORY)**:
   ...
   - **15m EMA CROSS CONFIRMATION**: Wait for clear separation (>0.1%) between fast and slow EMAs

4. **CONFIDENCE SCORING UPDATE**:
   - Base confidence on actual setup quality, not arbitrary numbers
   - Winning trades: Typically 65-70 confidence
   - Losing trades: Also 65-70 confidence
...
5. **ENTRY TIMING FILTERS**:
   - Avoid entering when 4h MACD histogram > 400 (exhaustion)
...
6. **PROFITABILITY RULES**:
   ...
   - **SOLUTION**: Use wider stops (0.8-1.2%) for high-quality LONG setups in bullish markets
   - **SOLUTION**: Use tighter stops (0.5-0.8%) for SHORT trades and BEARISH market trades
   - **SOLUTION**: Take partial profits at 0.9-1.2% for high-quality LONG trades
...
- **AVOID**: 4h MACD_hist > 400 (exhaustion risk - losing pattern)
...
- **DATA RULE**: 15m MACD_hist between -60 and +50 (optimal: -30 to +20)
- **DATA RULE**: 15m EMA alignment MUST be bullish for best results
- **DATA RULE**: Avoid if 15m MACD_hist < -60 (oversold) or > 100 (overextended)
...
- **DATA RULE**: 15m MACD_hist between -50 and +40 (optimal)
- **DATA RULE**: For counter-trend SHORT in bullish market, REQUIRE 15m EMA bearish alignment AND 4h MACD_hist < 100
- **DATA RULE**: Avoid if 15m MACD_hist > 100 (overbought reversal risk)
...
- **DATA RULE**: Avoid trades where 15m EMA fast and slow are within 0.05% of each other (indecision)
- **DATA RULE**: Require at least 0.1% separation between 15m EMAs for clear direction
...
- **DATA RULE**: Avoid extremes: 15m MACD_hist > 100 or < -60
- **DATA RULE**: 4h MACD_hist > 400 = avoid LONG (exhaustion - losing pattern)
- **DATA RULE**: 4h MACD_hist < -400 = avoid SHORT (oversold bounce)
- **DATA RULE**: For LONG in bullish market, 4h MACD_hist should be positive but < 400
- **DATA RULE**: For SHORT in bullish market, 4h MACD_hist should be < 100
...
5. **APPLY DATA RULE**: If 4h MACD_hist > 400, avoid LONG
...
4. **APPLY DATA RULE**: For SHORT in bullish market, verify 15m EMA bearish AND 4h MACD_hist < 100
...
- **SHORT trades in BULLISH markets**: 0.5-0.8% above entry
...
- **Partial profits**: Take 50% at 0.9-1.2% profit for high-quality LONG trades
...
1. **LONG in BULLISH markets** with:
   - 15m EMA bullish alignment (fast > slow by >0.1%)
   - 15m MACD_hist between -30 and +20
   - 4h MACD_hist positive but < 300
   - RSI(15m) between 35-45

2. **SHORT in BULLISH markets** (counter-trend) with:
   - 15m EMA bearish alignment (fast < slow by >0.1%) - REQUIRED
   - 4h MACD_hist < 100
   - 15m MACD_hist between -40 and +20
   - RSI(15m) > 55

### Avoid These Losing Patterns (From Data):
1. **LONG when 4h MACD_hist > 400** (exhaustion - high losing probability)
2. **Trading against 15m EMA alignment** (inconsistent results)
3. **Entering when 15m MACD_hist at extremes** (>100 or <-60)
