...
**ANALYSIS**: 7/7 winning trades were LONG. 0/3 SHORT trades were winners. LONG has 100% win rate in sample.
...
### 6. MACD HISTOGRAM STRENGTH: >50 REQUIRED (UPDATED)
**ANALYSIS**: Winning trades had MACD histogram 54.34-159.02 (avg 100.7). Losing trades often <50.
- **REQUIRE FOR LONG**: MACD histogram > 50 (positive momentum)
...
- **RATIONALE**: Positive MACD histogram (>50) indicates healthy momentum continuation

### 7. 15m EMA BULLISH ALIGNMENT WITH GAP >0.2% (UPDATED)
...
- **ADDITIONAL**: EMA gap should be > 0.2% of price (meaningful separation)
- **PREFER**: EMA gap 0.3-0.8% for strongest alignment
- **REJECT LONG**: EMA gap < 0.2% (tangled/weak alignment)
- **RATIONALE**: Clear EMA separation (>0.2%) confirms strong short-term momentum
...
5. ✅ MACD histogram > 50 (positive momentum)
6. ✅ 15m EMA alignment bullish with gap >0.2%
...
**Score each setup (must score ≥7.0 to enter):**
...
6. 15m EMA bullish with gap >0.2%: +1.0 (bonus +0.5 if gap >0.3%)
7. Daily bullish: +1.0 (if neutral: +0.5)
8. Risk-reward ≥1:1.25: +1.0 (bonus +0.5 if ≥1:1.5)

**ENTRY DECISION:**
- **Score ≥7.5**: ENTER FULL SIZE (confidence 75)
- **Score 7.0-7.4**: ENTER HALF SIZE (confidence 70)
- **Score <7.0**: REJECT (do not enter)

### 13. POSITION SIZING ADJUSTMENTS (ENHANCED)
**Based on confluence strength:**
- **BASE SIZE (100%)**: All 7 core conditions met, score ≥7.5
- **REDUCE 50%**: Score 7.0-7.4 OR 4H RSI 71-72 OR bb_pos > 108% OR RSI(15m) > 76
- **REDUCE 75%**: If 2+ caution conditions present
- **INCREASE 25%**: If ALL optimal: 4H RSI 65-70, bb_pos 100-106%, MACD hist > 75, Daily bullish, EMA gap >0.3%
...
- **USE 70**: For borderline setups (score 7.0-7.4)
- **USE 75**: For perfect setups (score ≥7.5)
...
4. ❌ 4H RSI < 57 OR > 73 (weak trend or extreme)
5. ❌ MACD histogram ≤ 50 (weak momentum)
6. ❌ 15m EMA alignment not bullish OR gap <0.2%
7. ❌ Daily timeframe bearish (immediate rejection)
8. ❌ Risk-reward < 1:1.25
9. ❌ Entry quality score < 7.0

### 18. BACKTEST PERFORMANCE INSIGHTS
- **Total trades**: 17 (7 wins, 10 losses)
- **Win rate**: 41.2% overall
- **LONG win rate**: 53.8% (7/13 in sample)
...
- **Key insight**: Only trade LONG when ALL 7 core conditions met with score ≥7.0
...
- MACD histogram >50 (vs losing often <50)
- EMA gap >0.2% (vs losing often <0.2%)
...
### 20. REMEMBER
This is now a **STRICT LONG-ONLY MOMENTUM CONTINUATION** strategy. Only enter LONG positions when ALL 7 core conditions are met AND entry quality score ≥7.0. Never take SHORT positions. Use 2.0% SL, 2.5% TP with trailing stops. The updated thresholds (bb_pos ≥95%, 4H RSI ≥57, MACD >50, EMA gap >0.2%) are based on actual winning trade minimums, not arbitrary optimizations.