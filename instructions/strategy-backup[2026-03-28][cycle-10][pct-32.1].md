...
<!-- KEY INSIGHT: Positive P&L ($68.90) despite low win rate indicates large average win size -->
<!-- CRITICAL FINDING: 15m EMA alignment with trade direction is the strongest predictor of success -->
<!-- NEW FINDING: 4h MACD histogram extremes (>300) are strong reversal signals -->
<!-- NEW FINDING: Counter-trend SHORT trades in BULLISH markets can be profitable with specific conditions -->
<!-- NEW FINDING: "5m:BUY 15m:BUY 1h:BUY" appears in 100% of sample trades - not a useful filter -->
<!-- NEW FINDING: 15m MACD_hist between -30 and +15 optimal for LONG entries -->
<!-- NEW FINDING: 15m EMA separation >0.08% required for clear direction -->
<!-- NEW FINDING: Avoid trades when 15m EMAs within 0.03% of each other (indecision) -->
...
1. **Win Rate**: 34.6% (84 wins, 159 losses) - low win rate but positive P&L
2. **Total P&L**: $68.90 (positive despite low win rate)
3. **Critical Insight**: Average win size significantly larger than average loss size (estimated 3:1 ratio)
4. **Performance Patterns Identified**:
   - **LONG trades in BULLISH markets**: Most profitable setup (multiple wins > $10, up to $29.06)
   - **SHORT trades in BULLISH markets**: Can work with specific conditions (11.41-14.46 wins observed)
   ...
   - **MACD Patterns**: Extreme 4h MACD_hist (>300) often leads to losses (observed in losing trades)
...
2. **15m MACD HISTOGRAM THRESHOLDS (PRECISE)**:
   - **For LONG entries**: MACD_hist between -30 and +15 (optimal: -20 to +5)
   - **For SHORT entries**: MACD_hist between -30 and +10 (optimal: -20 to 0)
   - **EXTREME VALUES**: >50 or <-30 often lead to immediate reversals
   - **WINNING PATTERN**: LONG with 15m MACD_hist slightly negative (-20 to -5) in bullish market
   - **LOSING PATTERN**: Entering when 15m MACD_hist is strongly positive (>40) for LONG or strongly negative (<-30) for SHORT
...
3. **EMA ALIGNMENT RULES (MANDATORY)**:
   ...
   - **CRITICAL**: For SHORT in bullish market, 15m EMA MUST show bearish alignment (fast < slow by >0.08%)
   - **REQUIRED**: Minimum 0.08% separation between 15m fast and slow EMA for any trade
...
4. **CONFIDENCE SCORING UPDATE**:
   ...
   - **NEW**: Confidence <65 for trades in bearish markets or weak setups
...
5. **ENTRY TIMING FILTERS**:
   ...
   - **NEW**: Require 15m EMA separation >0.08% for any trade entry
...
6. **PROFITABILITY RULES**:
   ...
   - **NEW**: Cut losses quickly on SHORT trades in bullish markets (smaller loss tolerance)
...
- **DATA RULE**: If 4h MACD_hist > 300, AVOID LONG (exhaustion risk - losing pattern)
- **DATA RULE**: If 4h MACD_hist negative in bullish market, caution - potential divergence
...
- **DATA RULE**: 15m MACD_hist between -30 and +15 (optimal: -20 to +5)
- **DATA RULE**: 15m EMA alignment MUST be bullish for best results
- **DATA RULE**: Avoid if 15m MACD_hist < -30 (oversold) or > 40 (overextended)
- **DATA RULE**: Require 15m EMA fast > slow by >0.08%
- **DATA RULE**: Avoid if 15m EMAs within 0.03% of each other
...
- **DATA RULE**: 15m MACD_hist between -30 and +10 (optimal: -20 to 0)
- **DATA RULE**: For counter-trend SHORT in bullish market, REQUIRE:
  - 15m EMA bearish alignment (fast < slow by >0.08%)
  - 4h MACD_hist < 150
  - 15m MACD_hist between -20 and 0
- **DATA RULE**: Avoid if 15m MACD_hist > 30 (overbought reversal risk)
- **DATA RULE**: Avoid if 15m EMAs within 0.03% of each other
...
- **DATA RULE**: Avoid extremes: 15m MACD_hist > 40 or < -30
...
7. ✅ 15m EMAs NOT within 0.03% of each other
...
- ❌ 15m MACD_hist outside optimal range (-30 to +15 for LONG, -30 to +10 for SHORT)
...
1. **LONG in BULLISH markets** with:
   ...
   - 15m MACD_hist between -20 and +5
...
3. **Entering when 15m MACD_hist at extremes** (>40 or <-30)
4. **Trades when 15m EMAs are too close** (<0.03% separation)
5. **Counter-trend SHORT when 4h MACD_hist strongly positive** (>200)
6. **LONG when 15m MACD_hist > 40** (overextended)
7. **SHORT when 15m MACD_hist < -30** (oversold bounce risk)
