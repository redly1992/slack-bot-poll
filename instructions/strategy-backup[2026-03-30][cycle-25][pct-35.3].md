<!-- AI-IMPROVED: 2026-03-30 | Cycle 24 -->
...
## 📊 LEARNED RULES (Cycle 24)

### 1. LONG-ONLY STRATEGY (CONFIRMED & ENFORCED)
**ANALYSIS**: 6/6 winning trades were LONG. 0/3 SHORT trades were winners. SHORT trades lost 100% of time with average loss of -3.20%.
...
- **REDUCE 50%**: Score 8.0-8.4 OR 4H RSI 70-72 OR bb_pos > 108% OR RSI(15m) > 73
...
### 15. EXIT STRATEGY OPTIMIZATION (NEW)
...
### 25. REMEMBER
This is now a **STRICT LONG-ONLY MOMENTUM CONTINUATION** strategy with **TIGHTER FILTERS**. Only enter LONG positions when ALL 7 core conditions are met with UPDATED THRESHOLDS (bb_pos 95-110%, RSI(15m) 67-76, 4H RSI 57-72, MACD >75, EMA gap >0.3%) AND entry quality score ≥8.0. Never take SHORT positions. Use 2.0% SL, 2.5% TP with trailing stops. The updated thresholds are based on actual winning trade minimums with buffer for safety.

---

## 📊 LEARNED RULES (Cycle 24 - IMPROVED)

### 1. LONG-ONLY STRATEGY (CONFIRMED)
**ANALYSIS**: 6/6 winning trades were LONG. 0/3 SHORT trades were winners.
- **REQUIRE**: Only take LONG positions
- **IMMEDIATE REJECTION**: ALL SHORT signals regardless of market condition
- **RATIONALE**: SHORT trades lost 100% of time with average loss -3.20%

### 2. OPTIMAL STOP LOSS: 2.0% (CONFIRMED)
**ANALYSIS**: All winning trades used 2.0% SL. 80% of losses exited via SL.
- **REQUIRE**: SL distance MUST be exactly 2.0% (entry × 0.98)
- **NO EXCEPTIONS**: Never use arbitrary SL placement
- **RATIONALE**: Standardized 2.0% SL provides consistent risk management

### 3. OPTIMAL TAKE PROFIT: 2.5% (CONFIRMED)
**ANALYSIS**: Winning trades averaged 2.30% gain. 71.4% of wins exited via TP.
- **SET TP**: 2.5% above entry (entry × 1.025)
- **TRAILING STOP**: Activate at 2.0% gain, trail at 1.0% below current
- **PARTIAL EXIT**: Take 50% at 2.5%, let 50% run with trailing stop

### 4. BOLLINGER BAND POSITION: 95-110% REQUIRED (REFINED)
**ANALYSIS**: Winning trades had bb_pos 95.6-110.0% (avg 101.5%).
- **REQUIRE**: bb_pos ≥ 95% AND ≤ 110%
- **OPTIMAL**: 100-108% for highest probability
- **REJECT**: If bb_pos < 95% OR > 110%
- **REDUCE SIZE 50%**: If bb_pos > 108%

### 5. 15m RSI: 67-76 RANGE REQUIRED (CONFIRMED)
**ANALYSIS**: Winning trades had RSI(15m) 67.4-76.3.
- **REQUIRE**: RSI(15m) ≥ 67 AND ≤ 76
- **OPTIMAL**: 70-76 for best continuation
- **REJECT**: If RSI(15m) < 67 OR > 76
- **REDUCE SIZE 50%**: If RSI(15m) > 73

### 6. 4H RSI: 57-72 RANGE REQUIRED (CRITICAL)
**ANALYSIS**: Winning trades had 4H RSI 57.7-76.5.
- **REQUIRE**: 4H RSI ≥ 57 AND ≤ 72
- **OPTIMAL**: 65-72 for strongest setups
- **REJECT**: If 4H RSI < 57 OR > 72
- **REDUCE SIZE 50%**: If 4H RSI 70-72

### 7. MACD HISTOGRAM STRENGTH: >75 REQUIRED (CONFIRMED)
**ANALYSIS**: Winning trades had MACD histogram 54.34-159.02 (avg 100.7).
- **REQUIRE**: MACD histogram > 75
- **PREFER**: MACD histogram > 100
- **REJECT**: If MACD histogram ≤ 75

### 8. 15m EMA BULLISH ALIGNMENT WITH GAP >0.3% (CONFIRMED)
**ANALYSIS**: All winning trades had bullish EMA alignment with gaps >0.17%.
- **REQUIRE**: 15m EMA alignment bullish (ema_fast > ema_slow)
- **REQUIRE**: EMA gap > 0.3% of price
- **PREFER**: EMA gap 0.4-0.8%
- **REJECT**: EMA gap < 0.3%
