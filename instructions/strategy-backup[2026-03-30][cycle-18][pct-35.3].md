...
### 18. REMEMBER
This is now a **STRICT LONG-ONLY MOMENTUM CONTINUATION** strategy. Only enter LONG positions when ALL 6 core conditions are met: 1) LONG only, 2) bb_pos ≥95%, 3) RSI(15m) ≥67, 4) 4H RSI ≥65, 5) MACD histogram >50, 6) 15m EMA bullish with gap >0.15%. Never take SHORT positions. Use 2.0% SL and let winners run with trailing stops. This strict filter should improve win rate significantly based on the 100% success rate of LONG trades meeting these criteria in the sample.