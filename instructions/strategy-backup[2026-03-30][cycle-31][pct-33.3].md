<!-- AI-IMPROVED: 2026-03-30 | Cycle 30 -->
...
### 2. AVOID EXTREME BOLLINGER BAND POSITIONS (VALIDATED)
...
### 3. OPTIMAL RSI RANGES FOR ENTRY (VALIDATED)
...
**ANALYSIS**: Losing trades had SL distances: 2.76%, 3.03%, 1.75%. The 1.75% was too tight and got stopped.
- **OPTIMAL SL distance**: 2.5-3.0% (adjusted from 2.8-3.5%)
...
- **RATIONALE**: The 1.75% SL was too tight and got hit by noise

### 5. TAKE PROFIT REALISTIC TARGETS (OPTIMIZED)
**ANALYSIS**: Losing trades had TP distances: 4.39%, 4.55%, 3.30% with R:R ratios: 1.59, 1.50, 1.89.
- **OPTIMAL TP distance**: 3.5-4.5% (adjusted from 4.0-5.0%)
...
**ANALYSIS**: Executed trades had confidence 45, 40, 65. All lost. The 65 confidence trade had 1.75% SL (too tight).
- **MINIMUM EXECUTION CONFIDENCE**: 70 (ADJUSTED from 75)
- **HIGH QUALITY**: 80+ (requires 5+ conditions met)
- **MEDIUM QUALITY**: 70-79 (4 conditions met)
- **LOW QUALITY**: <70 (DO NOT EXECUTE)
- **RATIONALE**: 75 was too restrictive, but 70 still filters poor setups
...
- **STRICT REJECT**: MACD hist < -100 or > 100 (extreme momentum - DO NOT TRADE)
...
- **REQUIRE FOR LONG**: Stochastic %K between 20 and 40 (optimal: 25-35) - NOT below 20
- **REQUIRE FOR SHORT**: Stochastic %K between 60 and 80 (optimal: 65-75) - NOT above 80
- **STRICT REJECT**: %K < 20 for LONG or > 80 for SHORT
- **ADDITIONAL**: %K must be turning in trade direction (rising for LONG, falling for SHORT)

### 9. ENTRY CHECKLIST - STRICT VERSION (CYCLE 30)
...
6. ✅ Stochastic %K in optimal range (20-40 for LONG, 60-80 for SHORT) and turning
7. ✅ SL distance 2.5-3.0%
8. ✅ TP distance 3.5-4.5%
9. ✅ R:R ratio ≥ 1:1.5
10. ✅ Confidence ≥ 70
...
- **High volatility (ATR > 4.0%)**: Use wider SL (3.0-3.5%), larger TP (4.0-5.0%)
- **Normal volatility (ATR 2.0-4.0%)**: Use standard SL (2.5-3.0%), TP (3.5-4.5%)
- **Low volatility (ATR < 2.0%)**: Use tighter SL (2.0-2.5%), smaller TP (3.0-4.0%)
...
- **Take profit**: Primary at 3.5%, secondary at 2.0% with trail
...
- **AVOID**: Extreme MACD hist (<-100 or >100) (2 losses had this)
- **AVOID**: SL distance < 2.5% (1 loss had 1.75% - too tight)
- **AVOID**: Confidence < 70 (2 losses had 40-45 confidence)

### 17. OPTIMAL PARAMETER RANGES (PRECISE)
**Based on loss analysis and adjustments:**
- **SL distance**: 2.8% optimal (range: 2.5-3.0%)
- **TP distance**: 4.0% optimal (range: 3.5-4.5%)
...
- **Stochastic LONG**: 30 optimal (range: 25-35)
- **Stochastic SHORT**: 70 optimal (range: 65-75)
...
4. ✅ Stochastic in optimal range (20-40 LONG, 60-80 SHORT) and turning
5. ✅ MACD hist in optimal range (> -50 LONG, < 50 SHORT) and improving
6. ✅ SL distance 2.5-3.0%
7. ✅ TP distance 3.5-4.5%
8. ✅ R:R ratio ≥ 1:1.5
9. ✅ Confidence ≥ 70
...
### 20. KEY INSIGHTS FROM LOSS ANALYSIS (CYCLE 30)
...
4. **SL distance matters**: 1.75% was too tight and got hit by noise
5. **Confidence threshold works**: The 40-45 confidence trades lost, 65 confidence had wrong SL
...
5. **Check Stochastic** (must be 20-40 LONG, 60-80 SHORT and turning - if not, confidence ≤ 50)
...
**From Trade 1 (LONG loss):**
- ❌ bb_pos -13.9% (extreme <20%) → REJECT
- ❌ RSI 30.0 (borderline <35) → REJECT
- ❌ Stochastic 11.60 (extreme <20) → REJECT
- ✅ MACD hist -57.36 (within > -50 range) → ACCEPTABLE
...
- ❌ Stochastic 3.76 (extreme <20) → REJECT
- ❌ MACD hist -161.34 (extreme < -100) → REJECT
- ❌ 15m EMA alignment bearish (but trade was LONG) → This was the fatal error
- ❌ Confidence 40 (<70) → REJECT

**From Trade 3 (SHORT loss):**
- ❌ bb_pos 102.5% (extreme >80%) → REJECT
- ❌ RSI 76.6 (extreme >65) → REJECT  
- ❌ Stochastic 94.89 (extreme >80) → REJECT
- ❌ MACD hist 152.65 (extreme >100) → REJECT
- ❌ 15m EMA alignment bullish (but trade was SHORT) → This was the fatal error
- ❌ SL distance 1.75% (<2.5%) → REJECT
...
- **TP distance**: 4.0% optimal (range: 3.5-4.5%)
- **R:R ratio**: 1:1.8 optimal (range: 1:1.5-1:2.2)
- **Confidence threshold**: 70+ to execute (ADJUSTED from 75)
- **Position size**: 0.05% of portfolio per trade (maintained)
- **Indicator ranges**: More realistic for crypto volatility

### 25. SUMMARY OF CRITICAL CHANGES FOR CYCLE 30:
...
4. **OPTIMIZED SL distance** - 2.5-3.0% (minimum 2.5%)
5. **ADJUSTED confidence threshold** - 70+ to execute (from 75)
6. **RELAXED MACD hist ranges** - > -50 for LONG, < 50 for SHORT (from > -30, < 30)
7. **SMALLER TP ranges** - 3.5-4.5% (from 4.0-5.0%)
