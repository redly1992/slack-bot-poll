...
### 1. MANDATORY 15m EMA ALIGNMENT MATCH
...
### 2. AVOID EXTREME INDICATOR READINGS
...
- **For LONG**: bb_pos < -10% (price breaking down through lower band)
- **For SHORT**: bb_pos > 100% (price breaking out above upper band)
- **ANALYSIS**: All losing trades had extreme readings (RSI 24.2, 29.97, 76.6; Stochastic 3.76, 11.60, 94.89; bb_pos -13.9%, 102.5%)

### 3. OPTIMAL INDICATOR RANGES FOR ENTRIES
...
- **For LONG**: 4H MACD hist > -100 (not strongly bearish)
- **For SHORT**: 4H MACD hist < 100 (not strongly bullish)
- **AVOID** entering LONG when 4H MACD hist < -200 (strong downtrend momentum)
- **AVOID** entering SHORT when 4H MACD hist > 200 (strong uptrend momentum)
...
- **OPTIMAL SL distance**: 0.9-1.2% (avoid <0.8% and >1.5%)
- **MINIMUM SL distance**: 0.85% (tight stops get hit by noise)
- **MAXIMUM SL distance**: 1.3% (wide stops increase risk per trade)
- **ANALYSIS**: Losing trades had SL distances of 0.61%, 0.74%, and 1.75% - all outside optimal range
...
- **AVOID** excessive R:R > 1:3.5 (indicates unrealistic targets)
- **ANALYSIS**: Losing trades had good R:R ratios (1.89-4.12) but still lost due to poor entry timing

### 7. ENTRY TIMING WITHIN 15m TREND
...
### 8. CONFIDENCE SCORING ADJUSTMENTS
**Apply these confidence modifiers:**
- **+25 points** if 15m EMA alignment matches direction (MANDATORY)
- **+15 points** if indicators in optimal ranges (not extreme)
- **+10 points** if 4H MACD histogram supports direction
- **-40 points** if 15m EMA alignment contradicts direction (REJECT)
- **-20 points** if indicators in extreme ranges
- **-15 points** if 4H MACD histogram strongly opposes direction

**Minimum confidence thresholds:**
- **TRADEABLE**: 75+ (requires all mandatory conditions)
- **AVOID**: <75 (do not trade)

### 9. MARKET CONDITION BIAS
**Prioritize these setups:**
- **LONG in BULLISH 4H market**: Focus on moderate pullbacks (RSI 32-42)
- **SHORT in BEARISH 4H market**: Focus on moderate rallies (RSI 58-68)
- **AVOID** trading against 4H MACD histogram strength
- **CONFIRM**: 4H RSI should be in moderate range (40-60) - not at extremes

### 10. ENTRY CHECKLIST (ALL REQUIRED FOR HIGH CONFIDENCE)
**Before entering ANY trade, verify:**
1. ✅ 15m EMA alignment matches trade direction (bullish for LONG, bearish for SHORT)
2. ✅ Indicators in optimal ranges (not extreme)
3. ✅ 4H MACD histogram not strongly opposing (> -100 for LONG, < 100 for SHORT)
4. ✅ Price within Bollinger Bands (bb_pos between -5% and 95%)
5. ✅ SL distance 0.9-1.2%
6. ✅ R:R ratio ≥ 1:1.8
